(ns shipyard.operation
  "OperationActor -- one shipbuilding operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (Shipyard Advisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Shipyard Manufacturing
  ...
  One graph run = one shipbuilding operation (intake ->
  ...
  decision to a human operator (the shipyard engineer). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:actuation/dispatch-block`/`:actuation/issue-
  class-evidence` ALWAYS reach this node when the governor is
  clean -- see `shipyard.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [shipyard.shipyardadvisor :as shipyardadvisor]
            [shipyard.governor :as governor]
...
(defn build
  ...
  `shipyard.store/Store`).
  opts:
    :advisor      -- an `shipyard.shipyardadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (shipyardadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Shipyard Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (shipyardadvisor/-advise advisor store request)]
            {:proposal p :audit [(shipyardadvisor/trace request p)]})))

      ;; Shipyard Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
